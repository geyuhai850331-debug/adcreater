...
| 后端框架 | yudao-cloud 单机版 (Spring Boot) | 复用认证/鉴权/用户/RBAC 基础设施 |
...
基于 yudao-cloud 模块结构，新增 5 个业务模块：

```
adcreater-server/
├── adcreater-server-common/          # 共享: 枚举、异常、工具类
├── adcreater-framework/              # yudao 框架层 (复用)
├── module-system/                    # yudao 系统模块 (复用)
│
├── module-ad/                        # ★ 广告制作模块
...
| `delivery_task` | module-delivery | 投放任务记录 |
| `template` | module-template | 广告模板 |
| `template_version` | module-template | 模板版本 (支持增量同步) |
| `ai_model_config` | module-ai | AI 模型配置 (API Key 加密存储) |
| `prompt_template` | module-ai | Prompt 模板 |
| `user_points_account` | module-billing | 用户点数账户 (1:1 system_users) |
| `points_transaction` | module-billing | 点数流水 (只 INSERT) |
| `usage_record` | module-billing | 用量记录 (按模型/用户/时间) |
...
ad_task (
    ...
    type VARCHAR(32) NOT NULL,      -- copy / image / video
    input_params JSON,              -- 请求参数
    result JSON,                    -- 生成结果 (URL 等)
    status VARCHAR(32) NOT NULL,    -- pending / processing / done / failed
...
ai_model_config (
    ...
    adapter_class VARCHAR(255) NOT NULL,  -- 全限定类名
    api_key VARCHAR(512) NOT NULL,        -- AES 加密存储
    endpoint_url VARCHAR(255),
    is_enabled TINYINT DEFAULT 1,
    priority INT DEFAULT 0,
    extra_config JSON                     -- 计费单价等扩展配置
);
...
prompt_template (
    ...
    template_content TEXT NOT NULL,       -- 含 {{variable}} 占位符
    variables JSON,                       -- 占位符列表及默认值
    category VARCHAR(32) NOT NULL,        -- copy / image / video / digital_human
    is_enabled TINYINT DEFAULT 1
);
...
points_transaction (
    id BIGINT PRIMARY KEY,
    user_id BIGINT NOT NULL,
    type VARCHAR(16) NOT NULL,    -- earn / consume / refund
    amount INT NOT NULL,
    balance_after INT NOT NULL,
    status VARCHAR(16) NOT NULL,  -- pending / confirmed / rolled_back
    biz_id BIGINT,                -- 关联任务 ID
    remark VARCHAR(255)
);
```
...
- **API Key 加密**: `ai_model_config.api_key` AES 加密存储
- **点数流水不可变**: `points_transaction` 只 INSERT 不 UPDATE
- **模板版本化**: `template` + `template_version` 支持增量同步
...
| POST | `/api/ad/creative/video` | 广告视频生成 | SSE |
...
- 响应格式: `{ "code": 0, "data": {...}, "message": "Success" }`（yudao `CommonResult`）
...
步骤 3: 视频生成 (AdVideoPanel)
  → 选择素材图片 → 配置时长/风格 → SSE 生成 → 预览
