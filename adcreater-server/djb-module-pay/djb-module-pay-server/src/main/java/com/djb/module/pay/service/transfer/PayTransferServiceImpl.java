package com.djb.module.pay.service.transfer;
...
import com.djb.module.pay.framework.pay.core.client.PayClient;
import com.djb.module.pay.framework.pay.core.client.dto.transfer.PayTransferRespDTO;
import com.djb.module.pay.framework.pay.core.client.dto.transfer.PayTransferUnifiedReqDTO;
...
import com.djb.module.pay.controller.admin.transfer.vo.PayTransferPageReqVO;
import com.djb.module.pay.dal.dataobject.app.PayAppDO;
...
import com.djb.module.pay.dal.mysql.transfer.PayTransferMapper;
import com.djb.module.pay.dal.redis.no.PayNoRedisDAO;
import com.djb.module.pay.enums.notify.PayNotifyTypeEnum;
import com.djb.module.pay.enums.transfer.PayTransferStatusEnum;
import com.djb.module.pay.framework.pay.config.PayProperties;
import com.djb.module.pay.service.app.PayAppService;
import com.djb.module.pay.service.channel.PayChannelService;
import com.djb.module.pay.service.notify.PayNotifyService;
...
public class PayTransferServiceImpl implements PayTransferService {
    ...
    public PayTransferCreateRespDTO createTransfer(PayTransferCreateReqDTO reqDTO) {
        ...
        if (transfer == null) {
            ...
            transfer.setAppId(channel.getAppId());
            transfer.setChannelId(channel.getId());
            transfer.setNo(no);
            ...
            updateObj.setStatus(PayTransferStatusEnum.WAITING.getStatus());
            transferMapper.updateByIdAndStatus(transfer.getId(), transfer.getStatus(), updateObj);
        }
        ...
        try {
            ...
            transferUnifiedReq.setOutTransferNo(transfer.getNo());
        ...
        PayTransferCreateRespDTO respDTO = new PayTransferCreateRespDTO();
        respDTO.setId(transfer.getId());
        respDTO.setChannelPackageInfo(unifiedTransferResp != null ? unifiedTransferResp.getChannelPackageInfo() : null);
        return respDTO;
    }
    ...
    private void notifyTransferProgressing(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        // 2. 更新状态
        PayTransferDO updateObj = new PayTransferDO();
        updateObj.setStatus(PayTransferStatusEnum.PROCESSING.getStatus());
        updateObj.setChannelPackageInfo(notify.getChannelPackageInfo());
    ...
    private void notifyTransferSuccess(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        // 2. 更新状态
        PayTransferDO successUpdate = new PayTransferDO();
        successUpdate.setStatus(PayTransferStatusEnum.SUCCESS.getStatus());
        successUpdate.setSuccessTime(notify.getSuccessTime());
        successUpdate.setChannelTransferNo(notify.getChannelTransferNo());
        successUpdate.setChannelNotifyData(JsonUtils.toJsonString(notify));
        int updateCounts = transferMapper.updateByIdAndStatus(transfer.getId(),
    ...
    private void notifyTransferClosed(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        // 2. 更新状态
        PayTransferDO closedUpdate = new PayTransferDO();
        closedUpdate.setStatus(PayTransferStatusEnum.CLOSED.getStatus());
        closedUpdate.setChannelTransferNo(notify.getChannelTransferNo());
        closedUpdate.setChannelNotifyData(JsonUtils.toJsonString(notify));
        closedUpdate.setChannelErrorCode(notify.getChannelErrorCode());
        closedUpdate.setChannelErrorMsg(notify.getChannelErrorMsg());
