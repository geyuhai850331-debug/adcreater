package com.djb.module.pay.service.demo;
...
import com.djb.module.pay.api.order.PayOrderApi;
...
import com.djb.module.pay.controller.admin.demo.vo.order.PayDemoOrderCreateReqVO;
import com.djb.module.pay.dal.dataobject.demo.PayDemoOrderDO;
import com.djb.module.pay.dal.mysql.demo.PayDemoOrderMapper;
...
import static com.djb.framework.common.util.servlet.ServletUtils.getClientIP;
...
public class PayDemoOrderServiceImpl implements PayDemoOrderService {
    ...
    public Long createDemoOrder(Long userId, PayDemoOrderCreateReqVO createReqVO) {
        ...
        // 1.2 插入 demo 订单
        PayDemoOrderDO demoOrder = new PayDemoOrderDO();
        demoOrder.setUserId(userId);
        demoOrder.setSpuId(createReqVO.getSpuId());
        demoOrder.setSpuName(spuName);
        demoOrder.setPrice(price);
        ...
        createReqDTO.setAppKey(PAY_APP_KEY);
        createReqDTO.setUserIp(getClientIP());
        createReqDTO.setUserId(userId);
        createReqDTO.setUserType(UserTypeEnum.ADMIN.getValue());
        createReqDTO.setMerchantOrderId(demoOrder.getId().toString());
        createReqDTO.setSubject(spuName);
        createReqDTO.setBody("");
        createReqDTO.setPrice(price);
        createReqDTO.setExpireTime(addTime(Duration.ofHours(2L))); // 支付的过期时间
        Long payOrderId = payOrderApi.createOrder(createReqDTO).getCheckedData();
        // 2.2 更新支付单到 demo 订单
        PayDemoOrderDO updateObj = new PayDemoOrderDO();
        updateObj.setId(demoOrder.getId());
        updateObj.setPayOrderId(payOrderId);
        payDemoOrderMapper.updateById(updateObj);
    ...
    public void updateDemoOrderPaid(Long id, Long payOrderId) {
        ...
        updateObj.setPayChannelCode(payOrder.getChannelCode());
        int updateCount = payDemoOrderMapper.updateByIdAndPayed(id, false, updateObj);
        if (updateCount == 0) {
    ...
    public void refundDemoOrder(Long id, String userIp) {
        ...
        createReqDTO.setAppKey(PAY_APP_KEY);
        createReqDTO.setUserIp(getClientIP()); // 支付应用
        createReqDTO.setUserId(order.getUserId());
        createReqDTO.setUserType(UserTypeEnum.ADMIN.getValue()); // 用户信息
        createReqDTO.setMerchantOrderId(String.valueOf(order.getId())); // 支付单号
        createReqDTO.setMerchantRefundId(refundId);
        createReqDTO.setReason("想退钱");
        createReqDTO.setPrice(order.getPrice()); // 价格信息
        Long payRefundId = payRefundApi.createRefund(createReqDTO).getCheckedData();
        // 2.3 更新退款单到 demo 订单
        PayDemoOrderDO updateRefundObj = new PayDemoOrderDO();
        updateRefundObj.setId(id);
        updateRefundObj.setPayRefundId(payRefundId);
        updateRefundObj.setRefundPrice(order.getPrice());
        payDemoOrderMapper.updateById(updateRefundObj);
    }
