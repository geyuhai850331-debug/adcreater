...
import com.djb.module.pay.api.transfer.PayTransferApi;
...
import com.djb.module.pay.controller.admin.demo.vo.withdraw.PayDemoWithdrawCreateReqVO;
import com.djb.module.pay.dal.dataobject.demo.PayDemoWithdrawDO;
import com.djb.module.pay.dal.mysql.demo.PayDemoWithdrawMapper;
import com.djb.module.pay.enums.PayChannelEnum;
import com.djb.module.pay.enums.demo.PayDemoWithdrawStatusEnum;
import com.djb.module.pay.enums.demo.PayDemoWithdrawTypeEnum;
...
public class PayDemoWithdrawServiceImpl implements PayDemoWithdrawService {
    ...
    public Long createDemoWithdraw(@Valid PayDemoWithdrawCreateReqVO reqVO) {
        ...
        withdraw.setTransferChannelCode(getTransferChannelCode(reqVO.getType()));
    ...
    public Long transferDemoWithdraw(Long id, Long userId) {
        ...
        if (PayDemoWithdrawStatusEnum.isClosed(withdraw.getStatus())) {
            PayDemoWithdrawDO resetWithdraw = new PayDemoWithdrawDO();
            resetWithdraw.setStatus(PayDemoWithdrawStatusEnum.WAITING.getStatus());
            resetWithdraw.setTransferErrorMsg("");
            int updateCount = demoTransferMapper.updateByIdAndStatus(withdraw.getId(), withdraw.getStatus(), resetWithdraw);
            if (updateCount == 0) {
        ...
        transferReqDTO.setAppKey(PAY_APP_KEY);
        transferReqDTO.setChannelCode(withdraw.getTransferChannelCode());
        transferReqDTO.setUserIp(getClientIP()); // 支付应用
        transferReqDTO.setUserId(userId);
        transferReqDTO.setUserType(UserTypeEnum.ADMIN.getValue()); // 用户信息
        transferReqDTO.setMerchantTransferId(String.valueOf(withdraw.getId())); // 业务的订单编号
        transferReqDTO.setSubject(withdraw.getSubject());
        transferReqDTO.setPrice(withdraw.getPrice()); // 价格信息
        transferReqDTO.setUserAccount(withdraw.getUserAccount());
        transferReqDTO.setUserName(withdraw.getUserName()); // 收款信息
        ...
        PayDemoWithdrawDO updateTransfer = new PayDemoWithdrawDO();
        updateTransfer.setPayTransferId(transferRespDTO.getId());
        demoTransferMapper.updateByIdAndStatus(withdraw.getId(), withdraw.getStatus(), updateTransfer);
    ...
    public void updateDemoWithdrawTransferred(Long id, Long payTransferId) {
        ...
        PayDemoWithdrawDO updateWithdraw = new PayDemoWithdrawDO();
        ...
        demoTransferMapper.updateByIdAndStatus(withdraw.getId(), withdraw.getStatus(), updateWithdraw);
    }
