package com.djb.module.pay.service.wallet;
...
import com.djb.module.pay.api.refund.PayRefundApi;
import com.djb.module.pay.api.refund.dto.PayRefundCreateReqDTO;
import com.djb.module.pay.api.refund.dto.PayRefundRespDTO;
import com.djb.module.pay.controller.app.wallet.vo.recharge.AppPayWalletRechargeCreateReqVO;
...
import com.djb.module.pay.dal.dataobject.wallet.PayWalletRechargePackageDO;
import com.djb.module.pay.dal.mysql.wallet.PayWalletRechargeMapper;
import com.djb.module.pay.enums.PayChannelEnum;
import com.djb.module.pay.enums.order.PayOrderStatusEnum;
import com.djb.module.pay.enums.refund.PayRefundStatusEnum;
import com.djb.module.pay.enums.wallet.PayWalletBizTypeEnum;
import com.djb.module.pay.framework.pay.config.PayProperties;
import com.djb.module.pay.service.order.PayOrderService;
import com.djb.module.system.api.social.SocialClientApi;
import com.djb.module.system.api.social.dto.SocialWxaOrderUploadShippingInfoReqDTO;
import com.djb.module.system.api.social.dto.SocialWxaSubscribeMessageSendReqDTO;
...
import static com.djb.framework.common.exception.util.ServiceExceptionUtil.exception;
import static com.djb.framework.common.util.date.LocalDateTimeUtils.addTime;
import static com.djb.framework.common.util.json.JsonUtils.toJsonString;
import static com.djb.framework.common.util.number.MoneyUtils.fenToYuanStr;
import static com.djb.module.pay.convert.wallet.PayWalletRechargeConvert.INSTANCE;
import static com.djb.module.pay.enums.ErrorCodeConstants.*;
import static com.djb.module.pay.enums.MessageTemplateConstants.WXA_WALLET_RECHARGER_PAID;
import static com.djb.module.pay.enums.refund.PayRefundStatusEnum.*;
...
public class PayWalletRechargeServiceImpl implements PayWalletRechargeService {
    ...
    public PayWalletRechargeDO createWalletRecharge(Long userId, Integer userType, String userIp,
        ...
        payOrderCreateReqDTO.setMerchantOrderId(recharge.getId().toString());
        ...
        payOrderCreateReqDTO.setExpireTime(addTime(Duration.ofHours(2L))); // TODO @DJB：支付超时时间
        Long payOrderId = payOrderService.createOrder(payOrderCreateReqDTO);
        // 2.2 更新钱包充值记录中支付订单
        PayWalletRechargeDO updateRecharge = new PayWalletRechargeDO();
    ...
    public void updateWalletRechargerPaid(Long id, Long payOrderId) {
        ...
        int updateCount = walletRechargeMapper.updateByIdAndPaid(id, false,
                buildPaidWalletRecharge(id, payOrderDO.getChannelCode()));
        if (updateCount == 0) {
    ...
    public void sendWalletRechargerPaidMessage(Long payOrderId, PayWalletRechargeDO walletRecharge) {
        ...
        SocialWxaSubscribeMessageSendReqDTO messageReqDTO = new SocialWxaSubscribeMessageSendReqDTO();
        messageReqDTO.setUserId(wallet.getUserId());
        messageReqDTO.setUserType(wallet.getUserType());
        messageReqDTO.setTemplateTitle(WXA_WALLET_RECHARGER_PAID);
        messageReqDTO.setPage("pages/user/wallet/money"); // 钱包详情界面
        messageReqDTO.addMessage("character_string1", String.valueOf(payOrderId)); // 支付单编号
        messageReqDTO.addMessage("amount2", fenToYuanStr(walletRecharge.getTotalPrice())); // 充值金额
        messageReqDTO.addMessage("time3", LocalDateTimeUtil.formatNormal(walletRecharge.getCreateTime())); // 充值时间
        messageReqDTO.addMessage("phrase4", "充值成功"); // 充值状态
        socialClientApi.sendWxaSubscribeMessage(messageReqDTO).checkError();
        ...
        reqDTO.setTransactionId(payOrder.getChannelOrderNo());
        reqDTO.setOpenid(payOrder.getChannelUserId());
        reqDTO.setItemDesc(payOrder.getSubject());
        reqDTO.setLogisticsType(SocialWxaOrderUploadShippingInfoReqDTO.LOGISTICS_TYPE_VIRTUAL); // 虚拟物品发货类型
        try {
    ...
    private PayWalletRechargeDO buildPaidWalletRecharge(Long id, String payChannelCode) {
        PayWalletRechargeDO updateObj = new PayWalletRechargeDO();
        updateObj.setId(id);
        updateObj.setPayStatus(true);
        updateObj.setPayTime(LocalDateTime.now());
        updateObj.setPayChannelCode(payChannelCode);
        return updateObj;
    }
    ...
    public void refundWalletRecharge(Long id, String userIp) {
        ...
        PayRefundCreateReqDTO refundCreateReqDTO = new PayRefundCreateReqDTO();
        refundCreateReqDTO.setAppKey(payProperties.getWalletPayAppKey());
        refundCreateReqDTO.setUserIp(userIp);
        refundCreateReqDTO.setUserId(wallet.getUserId());
        refundCreateReqDTO.setUserType(wallet.getUserType());
        refundCreateReqDTO.setMerchantOrderId(walletRechargeId);
        refundCreateReqDTO.setMerchantRefundId(refundId);
        refundCreateReqDTO.setReason("想退钱");
        refundCreateReqDTO.setPrice(walletRecharge.getPayPrice());
        Long payRefundId = payRefundApi.createRefund(refundCreateReqDTO).getCheckedData();

        // 4. 更新充值记录退款单号
        PayWalletRechargeDO updateObj = new PayWalletRechargeDO();
        updateObj.setPayRefundId(payRefundId);
        updateObj.setRefundStatus(WAITING.getStatus());
        updateObj.setId(walletRecharge.getId());
        walletRechargeMapper.updateById(updateObj);
    }
    ...
    public void updateWalletRechargeRefunded(Long id, String refundId, Long payRefundId) {
        ...
        if (PayRefundStatusEnum.isSuccess(payRefund.getStatus())) {
            ...
            updateObj.setRefundStatus(SUCCESS.getStatus());
    ...
    private PayWalletDO validateWalletRechargeCanRefund(PayWalletRechargeDO walletRecharge) {
        ...
        // TODO @DJB：需要考虑下，赠送的金额，会不会导致提现超过；
        return wallet;
    }
