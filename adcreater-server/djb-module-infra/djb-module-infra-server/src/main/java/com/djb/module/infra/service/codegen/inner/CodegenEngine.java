package com.djb.module.infra.service.codegen.inner;
...
import com.djb.framework.apilog.core.annotation.ApiAccessLog;
import com.djb.framework.apilog.core.enums.OperateTypeEnum;
...
import com.djb.framework.common.util.date.DateUtils;
import com.djb.framework.common.util.date.LocalDateTimeUtils;
import com.djb.framework.common.util.object.BeanUtils;
import com.djb.framework.common.util.object.ObjectUtils;
import com.djb.framework.common.util.string.StrUtils;
import com.djb.framework.excel.core.annotations.DictFormat;
import com.djb.framework.excel.core.convert.DictConvert;
import com.djb.framework.excel.core.util.ExcelUtils;
import com.djb.framework.mybatis.core.dataobject.BaseDO;
...
import com.djb.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 djb-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 djb-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.djb.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/djb-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/djb-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "djb-module-${table.moduleName}/" + // 顶级模块
                "djb-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "djb-module-${table.moduleName}/" + // 顶级模块
                "djb-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "djb-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "djb-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "djb-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "djb-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
