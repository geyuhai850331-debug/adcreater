package com.djb.module.member.service.user;
...
import com.djb.module.member.controller.admin.user.vo.MemberUserPageReqVO;
import com.djb.module.member.controller.admin.user.vo.MemberUserUpdateReqVO;
import com.djb.module.member.controller.app.user.vo.*;
import com.djb.module.member.convert.auth.AuthConvert;
import com.djb.module.member.convert.user.MemberUserConvert;
import com.djb.module.member.dal.dataobject.user.MemberUserDO;
import com.djb.module.member.dal.mysql.user.MemberUserMapper;
import com.djb.module.member.mq.producer.user.MemberUserProducer;
...
import com.djb.module.system.api.social.dto.SocialWxPhoneNumberInfoRespDTO;
...
import static com.djb.module.member.enums.ErrorCodeConstants.*;
...
public class MemberUserServiceImpl implements MemberUserService {
    ...
    public void updateUserMobile(Long userId, AppMemberUserUpdateMobileReqVO reqVO) {
        ...
        if (StrUtil.isNotEmpty(reqVO.getOldCode())) {
            SmsCodeUseReqDTO useOldCodeReqDTO = new SmsCodeUseReqDTO();
            useOldCodeReqDTO.setMobile(user.getMobile());
            useOldCodeReqDTO.setCode(reqVO.getOldCode());
            useOldCodeReqDTO.setScene(SmsSceneEnum.MEMBER_UPDATE_MOBILE.getScene());
            useOldCodeReqDTO.setUsedIp(getClientIP());
            smsCodeApi.useSmsCode(useOldCodeReqDTO).checkError();
        }
        // 2.2 使用新验证码
        SmsCodeUseReqDTO useNewCodeReqDTO = new SmsCodeUseReqDTO();
        useNewCodeReqDTO.setMobile(reqVO.getMobile());
        useNewCodeReqDTO.setCode(reqVO.getCode());
        useNewCodeReqDTO.setScene(SmsSceneEnum.MEMBER_UPDATE_MOBILE.getScene());
        useNewCodeReqDTO.setUsedIp(getClientIP());
        smsCodeApi.useSmsCode(useNewCodeReqDTO).checkError();
    ...
    public void updateUserPassword(Long userId, AppMemberUserUpdatePasswordReqVO reqVO) {
        ...
        MemberUserDO user = validateUserExists(userId);
        // 校验验证码
        SmsCodeUseReqDTO usePasswordCodeReqDTO = new SmsCodeUseReqDTO();
        usePasswordCodeReqDTO.setMobile(user.getMobile());
        usePasswordCodeReqDTO.setCode(reqVO.getCode());
        usePasswordCodeReqDTO.setScene(SmsSceneEnum.MEMBER_UPDATE_PASSWORD.getScene());
        usePasswordCodeReqDTO.setUsedIp(getClientIP());
        smsCodeApi.useSmsCode(usePasswordCodeReqDTO).checkError();
