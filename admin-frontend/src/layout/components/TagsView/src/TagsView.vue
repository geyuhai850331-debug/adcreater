...
// 根据当前语言获取显示标题
const getDisplayTitle = (item: RouteLocationNormalizedLoaded) => {
  const currentLang = localeStore.getCurrentLocale.lang
  // 英文环境下，优先显示 enName
  if (currentLang === 'en' && item?.meta?.enName) {
    return item.meta.enName + (item?.meta?.titleSuffix ? ` (${item?.meta?.titleSuffix})` : '')
  }
  return t(item?.meta?.title as string) + (item?.meta?.titleSuffix ? ` (${item?.meta?.titleSuffix})` : '')
}
